# YouTube Localization — Vivaldi Spring Mvt. I

> 영상 단위 현지화본 (제목 + 설명). YouTube Studio → 자막(Subtitles) → 언어 추가 → 제목 및 설명에 붙여넣기.
> base = 라이브 설명 (코튼 s339 paste). curator voice 양식 (Welcome 블록 대신). 핸들·채널명·© 라인은 전 로케일 공통 keep.
> 박힌 날짜: 2026-05-21 (s339) · 해시태그 v11 update (s351 — 로케일-퓨어 재설계) · 제목 update (s355 — `(feat. 初音ミク)` 후치 양식 · 브래킷 폐기)
> ⚠️ 라이브 ↔ repo drift: 라이브엔 (1) Welcome 블록 대신 curator voice 1줄, (2) 'No AI for music…' 줄 없음 — repo description.md 정본과 다름. (3) **해시태그 = v11(s351) 로케일-퓨어** (전 로케일 공통 아님 · 라이브는 Studio 수동 교체 대기). 본문은 라이브 충실 + 해시태그는 v11.

## 🇯🇵 日本語 (Japanese)

### 제목 (Title)
```
ヴィヴァルディ - 四季 春 第1楽章 (feat. 初音ミク)
```
<!-- s355 양식 (코튼 결단): 작곡가 풀네임 - 곡명 (feat. 初音ミク) 후치 양식 · 브래킷/아카펠라 폐기 (reveal 설계). feat. 크레딧 = 전 로케일 初音ミク (정체성 마크). 곡명 현지화(四季 春 第1楽章) keep. 라이브 적용 완료. -->


### 설명 (Description)
```
春の涼やかな風を待ちわびて選んだ一曲—
アントニオ・ヴィヴァルディ -「春」第1楽章（1725年） — 6人のミクが歌います。

カバーアート：ボッティチェッリの絵画「春（プリマヴェーラ）」（1482年頃）をもとに。
https://commons.wikimedia.org/wiki/File:Botticelli-primavera.jpg

チャンネル登録で「アトリエ」の仲間に。クラシックの新しい一面を、一緒に見つけましょう！

—

制作ツール：VOCALOID6 / ボイスバンク：初音ミクV6
初音ミク © Crypton Future Media, Inc.

#初音ミク #アカペラ #四季 #ボカロ #ボカロカバー #クラシック #バロック #ヴィヴァルディ #AtelierMikuAcappella
```

## 🇰🇷 한국어 (Korean)

### 제목 (Title)
```
비발디 - 사계 '봄' 1악장 (feat. 初音ミク)
```

### 설명 (Description)
```
봄의 선선한 바람을 그리며 고른 한 곡—
안토니오 비발디 - '봄' 1악장 (1725년) — 여섯 명의 미쿠가 노래합니다.

커버 아트: 보티첼리의 '봄(프리마베라)' (1482년경)을 바탕으로.
https://commons.wikimedia.org/wiki/File:Botticelli-primavera.jpg

구독하고 아틀리에의 일원이 되어, 클래식의 새로운 면을 함께 발견해요!

—

제작툴: VOCALOID6 / 보이스뱅크: 하츠네 미쿠 V6
하츠네 미쿠 © Crypton Future Media, Inc.

#하츠네미쿠 #아카펠라 #사계 #初音ミク #보컬로이드 #클래식 #바로크 #비발디 #AtelierMikuAcappella
```
