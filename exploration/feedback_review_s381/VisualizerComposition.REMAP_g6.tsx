...
const SPECTRUM_GAIN = 6.0; // compensate finer-bin normalization (tune via render)
...
const AMPLIFICATION_HIGH = 2.0; // flattened (was 6.0): band is energy-balanced now
...
export const VisualizerComposition: React.FC<VisualizerProps> = ({
  ...
  // Re-bucket the linear FFT into log-spaced display bars over [fLo, fHi].
  const binHz = audioData.sampleRate / (FFT_SAMPLES * 2);
  const logBars = (fLo: number, fHi: number, n: number): number[] => {
    const out: number[] = [];
    for (let k = 0; k < n; k++) {
      const f0 = fLo * Math.pow(fHi / fLo, k / n);
      const f1 = fLo * Math.pow(fHi / fLo, (k + 1) / n);
      const iLo = Math.max(0, Math.floor(f0 / binHz));
      const iHi = Math.min(visualization.length - 1, Math.ceil(f1 / binHz));
      let v = 0;
      for (let i = iLo; i <= iHi; i++) v = Math.max(v, visualization[i]);
      // linear interp at band center for sub-bin-width (low) bars
      const c = (f0 + f1) / 2 / binHz;
      const ci = Math.floor(c);
      if (ci + 1 < visualization.length) {
        const frac = c - ci;
        v = Math.max(
          v,
          visualization[ci] * (1 - frac) + visualization[ci + 1] * frac
        );
      }
      out.push(Math.min(1, v * SPECTRUM_GAIN));
    }
    return out;
  };

  const leftBarAmplitudes = logBars(BAND_LOW_HZ, BAND_SPLIT_HZ, BAR_COUNT_PER_SIDE);
  const rightBarAmplitudes = logBars(BAND_SPLIT_HZ, BAND_HIGH_HZ, BAR_COUNT_PER_SIDE);
