# YouTube Localization — Gymnopédie No. 1

> 영상 단위 현지화본 (제목 + 설명). YouTube Studio → 자막(Subtitles) → 언어 추가 → 제목 및 설명에 붙여넣기.
> base = 라이브 설명 (코튼 s339 paste). Welcome 블록 양식 유지 (조플린·비발디는 curator voice). 핸들·채널명·© 라인은 전 로케일 공통 keep.
> 박힌 날짜: 2026-05-21 (s339) · 해시태그 v11 update (s351 — 로케일-퓨어 재설계) · 제목 update (s355 — `(feat. 初音ミク)` 후치 양식 · 브래킷 폐기)
> ⚠️ 라이브 ↔ repo drift: (1) 라이브엔 'No AI for music…' 줄 없음 (repo description.md 정본엔 있음). (2) **해시태그 = v11(s351) 로케일-퓨어** (공유 이중언어 블록 폐기 → 로케일별 네이티브 · 전 로케일 공통 아님). 라이브는 Studio 수동 끝줄 교체 대기.

## 🇯🇵 日本語 (Japanese)

### 제목 (Title)
```
サティ - ジムノペディ第1番 (feat. 初音ミク)
```

### 설명 (Description)
```
Atelier Miku Acappella へようこそ！

クラシックの名曲を初音ミクの歌声でアカペラ編曲するファンプロジェクトです。一曲ずつ。
心に響く一曲が、ここで見つかりますように ♪

—

エリック・サティ - ジムノペディ第1番（1888年）。7人のミクが歌います。

カバーアート：ホイッスラーの絵画「青と金色のノクターン：オールド・バターシー・ブリッジ」（1872–75年）をもとに。
https://www.tate.org.uk/art/artworks/whistler-nocturne-blue-and-gold-old-battersea-bridge-n01959

チャンネル登録で「アトリエ」の仲間に。クラシックの新しい一面を、一緒に見つけましょう！

—

制作ツール：VOCALOID6 / ボイスバンク：初音ミクV6
初音ミク © Crypton Future Media, Inc.

#初音ミク #アカペラ #ジムノペディ #ボカロ #ボカロカバー #クラシック #ロマン派 #サティ #AtelierMikuAcappella
```

## 🇰🇷 한국어 (Korean)

### 제목 (Title)
```
사티 - 짐노페디 1번 (feat. 初音ミク)
```

### 설명 (Description)
```
Atelier Miku Acappella에 오신 걸 환영해요!

클래식 명곡을 하츠네 미쿠의 목소리로 아카펠라 편곡하는 팬 프로젝트입니다. 한 곡씩.
마음에 닿는 한 곡을 여기서 만나시길 ♪

—

에릭 사티 - 짐노페디 1번 (1888년). 일곱 명의 미쿠가 노래합니다.

커버 아트: 휘슬러의 '파랑과 금빛의 녹턴: 올드 배터시 브리지' (1872–75년)를 바탕으로.
https://www.tate.org.uk/art/artworks/whistler-nocturne-blue-and-gold-old-battersea-bridge-n01959

구독하고 아틀리에의 일원이 되어, 클래식의 새로운 면을 함께 발견해요!

—

제작툴: VOCALOID6 / 보이스뱅크: 하츠네 미쿠 V6
하츠네 미쿠 © Crypton Future Media, Inc.

#하츠네미쿠 #아카펠라 #짐노페디 #初音ミク #보컬로이드 #클래식 #낭만주의 #사티 #AtelierMikuAcappella
```
