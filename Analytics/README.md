...
| `growth.csv` | **스톡 시계열** (총 구독자·조회·영상수 = 누적 눈금 · `source`=reconstructed/snapshot) · `growth`가 주 1회 append · `growth-backfill`이 API 소급으로 전면 재구성 (트랙 C) |
| `channel_analysis.xlsx` | **Excel 분석 워크북** (개요·영상별·추이[차트]·트래픽[차트]·지역검색·원자료) · CSV에서 재생성 |

- **운영 방식 — 3트랙 측정 (2026-06-17 2트랙 확정 · 2026-07-02 트랙 C 주간 스톡 추가)**: 측정 종류별로 빈도를 분리한다. 핵심 원리 = **지표를 스톡(누적 눈금)과 플로우(기간 활동)로 갈라, 시간척도를 각각 맞춘다.** 비싼 수동작업(Studio 비율)은 **月 1회**, 싼 자동 스톡은 **주 1회**, 곡별 코호트는 **발행마다**.
  ...
  **트랙 C — 주간 스톡 스냅샷** (성장 궤적 · API-only · 코튼 손 0 · 2026-07-02 신설): 매주 `python Analytics/youtube_analytics.py growth` 1회 → 누적 눈금(총 구독자·조회·영상수)만 `growth.csv`에 한 줄 append. **스톡엔 창이 없어 겹침·자기상관 문제가 원천적으로 없다** → 주간으로 촘촘히 찍어 성장 곡선의 기울기(성장률)와 기울기 변화(가속/감속)를 읽는다. `channels.list(mine=True)` 1콜 = 비용 무 · Studio 불필요. xlsx "추이" 시트가 snapshots.csv 과거 스톡과 **측정일 병합**해 하나의 연속 곡선으로 그린다.
  - **소급 재구성** `growth-backfill` (2026-07-02): 과거에 스톡을 안 찍은 구간은 **Analytics API `dimensions=day`로 일자별 소급** 가능 → 일별 구독증감·조회를 누적 합산해 채널 개설부터 곡선을 처음부터 복원(영상수는 `series_history.csv` release_date 계단함수). `growth.csv`를 통째로 재작성(재실행 안전). 끝단 2~3일은 API 지연이라 today는 실측 절대값(`source=snapshot`)으로 마감, 나머지는 `source=reconstructed`.
  - ⚠️ **조회수 지표 gotcha**: Data API `viewCount`(공개 카운터 = `channels.list`/과거 snapshots 스톡)와 Analytics `views`(집계 조회)는 **서로 다른 지표라 소급값이 실측 스냅샷과 5~18% 어긋날 수 있다**(공개 카운터가 배치 지연으로 뒤늦게 수렴 · 끝점에선 일치). 구독(±1)·영상수(정확)는 잘 정합. 즉 재구성 곡선은 **형태·추세는 정확, 조회 절대값은 근사** — 절대 조회는 실측 snapshots/Studio가 우선.

  - **왜 (윈도우/비율은) 月 1회인가**: 비율·플로우 메인 지표가 28일 윈도우라 이걸 *주간*으로 재면 인접 측정이 **~75% 겹쳐 = 자기상관 노이즈**(겹친 슬라이스 delta는 가짜). 月 1회 ≈ 윈도우 비겹침 = 독립 샘플 = 정직한 추세. (s437 실측: 6/5↔6/15 채널 "노출 −21%"가 윈도우 오버랩 아티팩트로 드러남.) ★ **주의**: 이 "月 1회" 규칙은 **윈도우 플로우/비율 지표(트랙 A)에 한한다.** 트랙 C 주간 스톡은 이 규칙의 **폐기가 아니라 확장** — 스톡은 창이 없어 겹침이 애초에 성립 안 하므로 주간이 정답. 나중에 "月1회 노트"만 보고 "주간 스톡 = 모순"으로 오독하지 말 것.
  - **수집 지표 2층**: ⓐ **누적 성장(스톡)**(총 구독자·총 조회수·영상수 = lifetime · Data API `channels.list(mine=True)` · OAuth `youtube.readonly`) = 채널 성장의 절대 곡선 → **트랙 C가 주간 수집**(`growth.csv`) · 리포트 §1 최상단 + xlsx "추이" 시트 곡선. ⓑ **28일 윈도우 활동량(플로우)**(그 기간 조회·신규구독·시청) = 최근 활성도(트랙 A). 둘은 별개 — 혼동 금지(예: 윈도우 "신규구독 +9"는 총 구독자 증감이 아님).
  - **28일 고정 vs 주간 스톡**: 7일 *윈도우*는 신곡 직후 스팟체크(트랙 B) 전용 — 트랙 A 추세선엔 안 섞는다(섞으면 가짜 delta). 반면 주간 *스톡*(트랙 C)은 창이 아니라 눈금이므로 섞임 문제 자체가 없다.
